...
import { ArrowLeft, Archive, Bell, Calendar, ChevronLeft, ImageIcon, Link, StepBack } from "lucide-react";
...
const CRWDS = [
  ...
  { name: "Women's Healthcare of At…", status: "Recently visited", image: "/maz.jpg" },
  { name: "Children's Healthcare", status: "Recently visited", image: "/hy.jpg" },
  ...
  {CRWDS.map((crwd) => (
    ...
    >
      ...
      <div>
        ...
        <div className="text-muted-foreground text-sm">{crwd.status}</div>
      </div>
    </div>
  ))}
</div>
        <div className="flex justify-center p-4">
          <Button variant="outline" className="w-40">See More</Button>
        </div>
          ...
          <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50">
            <div className="bg-card rounded-xl shadow-lg p-8 flex flex-col items-center gap-4 max-w-sm mx-4">
              ...
              <div className="text-xl font-semibold text-center">
                Your post has been created for <span className='text-primary font-bold'>{selectedCRWD.name}</span>!
              </div>
              <Button
  ...
  <div>
    ...
    />
      ...
      <div className="border-t px-6 py-4 flex flex-col gap-2 bg-background/80 backdrop-blur-md">
        ...
        <div className="text-muted-foreground text-sm italic">Create a link, photo, or event post</div>
      </div>
      <div className="h-20 md:hidden"/>
    </div>
  );
