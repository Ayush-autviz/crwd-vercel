    ...
    <div className="flex items-center h-12 md:hidden px-4 border-b bg-[#F6F7FB]">
      ...
      <div className="flex-1 px-2 pt-4">
        {mockCrwds.map((crwd) => (
          ...
          >
            <div onClick={() => router.push(`/groupcrwd`)} className="cursor-pointer w-10 h-10 rounded-lg bg-blue-600 flex items-center justify-center text-white font-bold text-lg mr-3">
              {crwd.avatar}
            </div>
            <div onClick={() => router.push(`/groupcrwd`)} className="cursor-pointer flex-1 min-w-0">
           ...
           <Link href={`/your-crwds/${crwd.id}`}>
           <button
              className="ml-2 text-xs font-medium  hover:underline"
              // onClick={() => router.push(`/your-crwds/${crwd.id}`)}
            >
              Manage
            </button>
           </Link>
